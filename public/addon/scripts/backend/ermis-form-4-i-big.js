var Ermis = function () {
    ...
    var initStatus = function (flag) {
        ...
        if (flag === 1) {//DEFAULT
            ...
            jQuery('input').not('[type=radio]').val("");
            jQuery('textarea').val("");
            $("#image").data("kendoUpload").clearAllFiles();
            jQuery.each(data.columns, function (k, col) {
                if (col.key === 'select' && jQuery('select[name = ' + col.field + ']').hasClass("droplist")) {
        ...
        if (!jQuerylink.data('lockedAt') || +new Date() - jQuerylink.data('lockedAt') > 300) {
            $.when(KendoUiConfirm(transText.are_you_sure, transText.message)).then(function (confirmed) {
                if (confirmed) {
                    ...
                    if (crit === true) {
                      var a = jQuery('#image-form').get(0);
                      if(a){
                        var b = a[0].files[0];
                        var fd = new FormData(); // XXX: Neex AJAX2
                         fd.append('image', b);
                         fd.append('data', JSON.stringify(obj));
                          RequestURLImage(Ermis.link+'-save', 'json', fd, function (result) {
                              if (result.status === true) {
                                room.emit('client-send-save', {data : result.data, d :dataId , a:1})
                                initSaveComplete({data : result.data, d :dataId ,a:2})
                              }
        ...
        jQuery.each(data.columns, function (k, col) {
            if (col.key === 'text' || col.key === 'password' || col.key === 'number') {
                ...
                if (jQuery('textarea[name = ' + col.field + ']').hasClass("editor")) {
                    var decoded = $("<div/>").html(dataItem[col.field]).text();
