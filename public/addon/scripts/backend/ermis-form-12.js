var Ermis = function () {
    ...
    var initTable = function (data) {
        ...
        var ArrayHeaders = []; var ArrayHeader = [];
        var len = Ermis.columns.length;
        $.each(Ermis.columns[len-1], function (index, item) {
        ...
        var ArrayColumn = []; var configArray = [];
        var len_data = data === null ? 4 : data.length;
        $.each(Ermis.columns[len - 1], function (index, item) {
            var columnsList = {}; var configList = {};
            if (item.type === 'number') {
                columnsList = {
                    data: item.id,
                ...
                configList = {
                    sourceColumn: index,
                    destinationRow: len_data,
                    destinationColumn: index,
                    type: 'sum',
                    forceNumeric: true
                }
                configArray.push(configList);
