var Ermis = function () {
    ...
    var initSaveComplete = function(rd){
      var grid = $kGrid.data("kendoTreeList");
      if (rd.d === null) {
          grid.dataSource.pushCreate(rd.data);
          jQuery.each(data.columns, function (k, v) {
              if (v.addoption === "true") {
          ...
          grid.dataSource.pushUpdate(rd.data);
          jQuery.each(data.columns, function (k, v) {
              if (v.addoption === "true" && obj.code !== rd.data.code || v.addoption === "true" && obj.name !== rd.data.name) {
                ...
                if (confirmed) {
                    ...
                    if (crit === true) {
                        ...
                        RequestURLWaiting(Ermis.link+'-save', 'json', postdata, function (result) {
                            if (result.status === true) {
                              room.emit('client-send-save', {data : result.data, d :dataId })
                              initSaveComplete({data : result.data, d :dataId })
                            }
    ...
    var initKendoUiTreelist = function () {
